...
Public Class markah_ulang_akademik_bm
    ...
    Private Function getSQL() As String
        ...
        tmpSQL = "SELECT kpmkv_pelajar_ulang.PelajarUlangID, kpmkv_pelajar_ulang.Tahun, kpmkv_pelajar_ulang.Semester, "
        tmpSQL += " kpmkv_pelajar_ulang.Sesi, kpmkv_pelajar_ulang.NamaMataPelajaran, kpmkv_pelajar_ulang.MarkahPB,"
        tmpSQL += " kpmkv_pelajar_ulang.MarkahPA, "
        tmpSQL += " kpmkv_pelajar_ulang.Gred, kpmkv_pelajar.Tahun, kpmkv_pelajar.Nama, kpmkv_pelajar.Mykad, "
        tmpSQL += " kpmkv_pelajar.AngkaGiliran, kpmkv_kursus.KodKursus, kpmkv_kelas.NamaKelas"
        tmpSQL += " FROM  kpmkv_pelajar_ulang LEFT OUTER JOIN"
        ...
        strWhere += " AND kpmkv_pelajar_ulang.KolejRecordID='" & lblKolejID.Text & "'"
    ...
    Protected Sub btnUpdate_Click(sender As Object, e As EventArgs) Handles btnUpdate.Click
        ...
        Try
            For i As Integer = 0 To datRespondent.Rows.Count - 1
                ...
                strSQL = "UPDATE kpmkv_pelajar_ulang SET MarkahPB='" & strPB & "', MarkahPA='" & strPA & "' "
                strSQL += " WHERE PelajarUlangID='" & datRespondent.DataKeys(i).Value.ToString & "'"
                strRet = oCommon.ExecuteSQL(strSQL)
                If strRet = "0" Then
                    divMsg.Attributes("class") = "info"
    ...
    Private Sub Akademik_markah()
        ...
        For i As Integer = 0 To datRespondent.Rows.Count - 1
            ...
            strSQL = "SELECT PB FROM kpmkv_matapelajaran WHERE NamaMataPelajaran='" & strMP & "' AND SUBSTRING(KodMataPelajaran,4,3)='" & strKodMP & "' "
            strSQL +=" And Tahun ='" & ddlTahun.Text & "'"
            PB = oCommon.getFieldValue(strSQL)

            strSQL = "SELECT PA FROM kpmkv_matapelajaran WHERE NamaMataPelajaran='" & strMP & "' AND SUBSTRING(KodMataPelajaran,4,3)='" & strKodMP & "'"
            strSQL += " AND Tahun='" & ddlTahun.Text & "'"
            ...
            If ddlSemester.Text = "4" And strpointerNama = "BahasaMelayu" Then
                ...
                strSQL = "Select p.PelajarID "
                strSQL += " From kpmkv_pelajar As p "
                strSQL += " Left Join kpmkv_kursus as k ON k.KursusID=p.KursusID"
                strSQL += " WHERE p.Mykad ='" & strMYKAD1 & "'"
                strSQL += "  And p.Semester ='1' AND p.Sesi='" & chkSesi.Text & "' "
                strSQL += " And k.KodKursus ='" & strKodKursus & "' "
                ...
                strSQL += " Left Join kpmkv_kursus as k ON k.KursusID=p.KursusID"
                strSQL += " WHERE p.Mykad ='" & strMYKAD1 & "'"
                strSQL += "  And p.Semester ='2' AND p.Sesi='" & chkSesi.Text & "' "
                strSQL += " And k.KodKursus ='" & strKodKursus & "' "
                ...
                strSQL += " Left Join kpmkv_kursus as k ON k.KursusID=p.KursusID"
                strSQL += " WHERE p.Mykad ='" & strMYKAD1 & "'"
                strSQL += "  And p.Semester ='3' AND p.Sesi='" & chkSesi.Text & "' "
                strSQL += " And k.KodKursus ='" & strKodKursus & "' "
                ...
                Dim ar_Kertas As String() = strRet.Split("|")
