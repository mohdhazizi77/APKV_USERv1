...
Public Class markah_ulang_vokasional
    ...
    Private Sub Vokasional_markah()
        ...
        For i As Integer = 0 To datRespondent.Rows.Count - 1
            ...
            Dim StrPointerMax As Integer
            Dim strGetGrade As String
            ...
            'get gred from kpmkv_pelajar_markah (gred asal sebelum mgulang)
            ...
            Else
                ...
                ''kena tambah if gred mengulang lagi rendah dari sebelum..ambik gred sebelum

                'get markah max base on gred asal before mengulang
                strSQL = "SELECT MarkahTo FROM kpmkv_gred WHERE jenis ='VOKASIONAL' AND gred ='" & strGredMarkah & "'"
                StrPointerMax = oCommon.getFieldValue(strSQL)

                If StrPointerMax >= Pointer Then
                    strGetGrade = strGredMarkah
                Else
                    strGetGrade = Gred
                End If


                strSQL = "UPDATE kpmkv_pelajar_ulang_vokasional SET Gred='" & strGetGrade & "' Where PelajarUlangVID='" & datRespondent.DataKeys(i).Value.ToString & "'"
                strSQL += " AND Tahun='" & ddlTahun.Text & "' AND Semester='" & ddlSemester.Text & "'"
                strSQL += " AND Sesi='" & chkSesi.Text & "' AND KursusID='" & ddlKodKursus.SelectedValue & "' AND NamaModul='" & strModul & "'"
                strRet = oCommon.ExecuteSQL(strSQL)
